...
from odoo.addons.dai_tgg.models.model_dict_folder.recursive_func import muon_xuat_dac_tinh_gi,f_ordered_a_model_dict,rECURSIVE_ADD_MODEL_NAME_TO_FIELD_ATTR,define_col_index,define_col_index,tim_type_cua_attr,check_xem_att_co_nam_ngoai_khong,write_get_or_create_title,check_col_index_match_xl_title
...
#             required =get_key_allow(field_attr, 'required',key_tram, False)       
    ...
    if required and val==False:
        ...
        get_or_create = False
        return 'break', False, get_or_create #sua 5
...
def create_instance (self, MODEL_DICT, sheet, row, merge_tuple_list,needdata, noti_dict, main_call_create_instance_model = False,
    ...
    of_fields = MODEL_DICT.get('fields')
    model_name = get_key_allow(MODEL_DICT, 'model', key_tram)
    if main_call_create_instance_model:
        needdata['vof_dict'] = of_fields   #sua 2   vof_dict
    x2m_fields = []
#     remove_all_or_just_add_one_x2m = True
    ...
    for field_name,field_attr  in MODEL_DICT['fields'].items():
        c,o,g = in_for_create_instance(self,field_name,field_attr,key_tram,needdata,row,sheet,
                           MODEL_DICT,not_create,workbook_copy,sheet_of_copy_wb,
                           merge_tuple_list,model_name,main_call_create_instance_model,noti_dict,
                           key_search_dict,update_dict,x2m_fields,some_dict)
        if c =='break':
