...
def ong_ba_(ws,f_name,fixups,needdata,row,dlcv_obj,source_member_ids='source_member_ids'):
#     alist = [(u'Ông: Nguyễn Đức Tứ',u'CV: Nhân viên'),(u'Ông: Nguyễn Đức Tính',u'CV: Sếp')]
#     row = fixups['ddbg']['range'][0] + 1
#     row = needdata['cr'] + offset
    nrow = 0
    for c,i in enumerate(getattr(dlcv_obj,source_member_ids)):
        nrow +=1
...
#         ws.write_merge(row + c,row + c,5,7,u'                 Đ/v: %s'%i.parent_id.name,normal_style)
    return nrow
def table_(ws,f_name,fixups,needdata,row,dlcv_obj,IS_SET_TT_COL=False,all_tot=False):
#     row = needdata['cr'] + offset
    nrow = download_ml_for_bb(dlcv_obj, worksheet=ws,row_index=row,IS_SET_TT_COL = IS_SET_TT_COL,all_tot=all_tot)
#     needdata['cr'] = row + nrow - 1
    return nrow

def to_trinh_(ws,f_name,fixups,needdata,row,dlcv_obj):
    return (u'Căn cứ vào tờ trình ' + dlcv_obj.totrinh_id.get_names_for_report() )if  dlcv_obj.totrinh_id else u''

def hom_nay_(ws,f_name,fixups,needdata,row,dlcv_obj):
    return u'Hôm nay, ngày: ' + convert_odoo_datetime_to_vn_str(dlcv_obj.date,format='%d/%m/%Y') + u', Tại: ' + dlcv_obj.noi_ban_giao.name
def tinh_trang_vat_tu_(ws,f_name,fixups,needdata,row,dlcv_obj):
    all_tot = set(dlcv_obj.move_line_ids.mapped('tinh_trang')) ==set(['tot']) 
    all_hong = set(dlcv_obj.move_line_ids.mapped('tinh_trang')) ==set(['hong']) 
    if all_tot:
        return u'Tình trạng vật tư : Tốt'
    if all_hong:
        return u'Tình trạng vật tư : Hỏng'
    return None
def ky_ten_ben_giao_(ws,f_name,fixups,needdata,row,dlcv_obj,source_member_ids='source_member_ids',is_not_show = False):
    if is_not_show:
        return None
    source_member_ids = getattr(dlcv_obj,source_member_ids)
...
def dai_dien_ben_t3_(ws,f_name,fixups,needdata,row,dlcv_obj,title_ben_thu_3='title_ben_thu_3'):
    title_ben_thu_3 = getattr(dlcv_obj,title_ben_thu_3).name
    if title_ben_thu_3:
...
def offset_ky_ten_ben_t4_(n):
    if n['instance_dict']['ky_ten_ben_t3']['val'] != None:
        ...
        offset = 5
    return offset
def xac_nhan_lanh_dao_(ws,f_name,fixups,needdata,row,dlcv_obj):
    if dlcv_obj.is_not_show_y_kien_ld:
        return None
    else:
...
#     merge_tuple_list =  readsheet.merged_cells
    ...
    fixups =[  
                    ...
                    ('so',{'range':[2,2,0,2],'val':u'Số: %s/%s-%s'%(dlcv_obj.stt_trong_bien_ban_in,dlcv_obj.ma_bien_ban,dlcv_obj.department_id.short_name), 'style':xlwt.easyxf(generate_easyxf(height=12, vert = 'center',horiz = 'center'))}),
                    ...
                    ('ky_ten_ben_giao',{'range': ['auto','auto',0,2],'val':None,'offset':5,'val_func':ky_ten_ben_giao_, 'style':bold_center_style}),
                    ('ky_ten_ben_nhan',{'range': ['auto','auto',4,7],'val':None,'offset':0,
                                        'val_func':ky_ten_ben_giao_,'val_kargs':{'source_member_ids':'dest_member_ids'}, 'style':bold_center_style}),
                    ...
                    ('ky_ten_ben_t3',{'range': ['auto','auto',0,2],'val':None,'offset':5,'val_func':ky_ten_ben_giao_, 'style':bold_center_style,'val_kargs':{'source_member_ids':'ben_thu_3_ids'}}),
                    ('ky_ten_ben_t4',{'range': ['auto','auto',4,7],'val':None,'offset':offset_ky_ten_ben_t4_,
                                        'val_func':ky_ten_ben_giao_, 'style':bold_center_style,'val_kargs':{'source_member_ids':'ben_thu_4_ids'}
                                        }, ),
...
#                                    'val_func':ky_ten_ben_giao_, 'style':bold_center_style,'val_kargs':{'source_member_ids':'lanh_dao_id','is_not_show':dlcv_obj.is_not_show_y_kien_ld}
#                                   }),
             ]
    ...
    for f_name,field_attr in fixups.items():
        ...
        if val_func:
            ...
            val = val_func(ws,f_name,fixups,needdata,row,dlcv_obj,**val_kargs)
        func = field_attr.get('func')
        if func:
            kargs = field_attr.get('kargs',{})
            nrow = func(ws,f_name,fixups,needdata,row,dlcv_obj, **kargs)
